...
class ResourceManager:
	...
	def add_sound_recording(self, upc, file_path, isrc, title, resource_reference):
		file_name = self.__file_name_from(isrc, file_path)
		moved_file_path = self.__copy_file(upc, file_path, file_name)
		return SoundRecording(resource_reference, isrc, title, self._file_parser.parse(moved_file_path), '')
	
	def add_image(self, upc, file_path, resource_reference):
		file_name = self.__file_name_from(upc, file_path)
		moved_file_path = self.__copy_file(upc, file_path, file_name)
		return Image(resource_reference, upc, self._file_parser.parse(moved_file_path), '')
